public class ListLinked<T extends Comparable<T>> {
    ...
    public ListLinked() { this.first = null; this.size = 0; }

    public Node<T> getFirstNode() { return first; }
    protected void setFirstNode(Node<T> node) { this.first = node; }
    protected void incrementSize() { this.size++; }

    public boolean isEmptyList() { return size == 0; }
    ...
    public void addLast(T dato) { insertLast(dato); }
    ...
    public boolean removeNode(T dato) {
        ...
        Node<T> aux = this.first;
        while (aux.next != null && aux.next.dato.compareTo(dato) != 0) aux = aux.next;
    ...
    public void print() {
        if (this.isEmptyList()) { System.out.println(""); return; }
